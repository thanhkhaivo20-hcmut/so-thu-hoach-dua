...
const App = () => {
  ...
  const [newCustomer, setNewCustomer] = useState({ name: "", standard_count: 120, cycle_days: 25 });
  ...
  const handleAddCustomer = () => {
    ...
    .then(data => {
      ...
      setNewCustomer({ name: "", standard_count: 120, cycle_days: 25 });
  ...
  const calculateReport = () => {
    let totalCoconuts = 0;
    let stats = {};
    customers.forEach(c => { stats[c.name] = { total_harvest: 0, standard_count: c.standard_count }; });
    Object.values(records).forEach(record => {
      if (record.status === 'recorded') {
        totalCoconuts += record.count;
        if (!stats[record.name]) stats[record.name] = { total_harvest: 0, standard_count: 120 };
        stats[record.name].total_harvest += record.count;
      }
    });
    return { totalCoconuts, stats };
  };

  const { totalCoconuts, stats } = calculateReport();

  if (!role) {
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center font-sans p-4">
        {/* Đổi width để vừa màn hình nhỏ */}
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-3 md:p-6 font-sans relative">
      {/* Thanh Header: Tự động rớt dòng trên điện thoại */}
      ...
      {activeTab === 'calendar' && (
        <>
          ...
          {/* Lưới lịch: Thu nhỏ padding và font-size trên mobile */}
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1 md:mt-2">
                    {/* Chỉ hiện 1 chữ cái tên khách hàng trên mobile cho gọn, hiện đầy đủ trên máy tính */}
        ...
        <div className="bg-white p-4 md:p-6 rounded-md shadow-sm">
          ...
          <div className="flex flex-col md:flex-row gap-2 mb-6 bg-gray-50 p-4 rounded border">
            ...
            <div className="flex gap-2">
              <input type="number" placeholder="Chuẩn (120)" className="border p-2 rounded w-1/2 md:w-32" value={newCustomer.standard_count} onChange={e => setNewCustomer({...newCustomer, standard_count: Number(e.target.value)})} />
              <input type="number" placeholder="Chu kỳ (25)" className="border p-2 rounded w-1/2 md:w-32" value={newCustomer.cycle_days} onChange={e => setNewCustomer({...newCustomer, cycle_days: Number(e.target.value)})} />
            </div>
          ...
          {/* Bọc table trong div overflow-x-auto để cuộn ngang trên mobile */}
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[400px]">
              <thead>
                <tr className="border-b bg-gray-50"><th className="p-2 md:p-3">Tên Khách Hàng</th><th className="p-2 md:p-3">Chuẩn</th><th className="p-2 md:p-3">Chu kỳ</th></tr>
              </thead>
              <tbody>
                {customers.map((c, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50"><td className="p-2 md:p-3 font-medium">{c.name}</td><td className="p-2 md:p-3">{c.standard_count}</td><td className="p-2 md:p-3">{c.cycle_days} ngày</td></tr>
                ))}
              </tbody>
            </table>
          </div>
      ...
      {activeTab === 'report' && (
        <div className="bg-white p-4 md:p-6 rounded-md shadow-sm">
          ...
          <div className="bg-blue-50 border border-blue-200 p-4 md:p-6 rounded-lg mb-6 flex flex-col md:flex-row justify-between items-center text-center md:text-left">
            <div className="mb-2 md:mb-0">
              <h3 className="text-base md:text-lg font-semibold text-blue-900">Tổng sản lượng</h3>
            </div>
            <div className="text-3xl md:text-4xl font-bold text-blue-600">
              {totalCoconuts.toLocaleString()} <span className="text-lg md:text-xl font-medium text-blue-800">trái</span>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[500px]">
              <thead>
                <tr className="border-b bg-gray-50 text-gray-700"><th className="p-2 md:p-3">Tên Khách Hàng</th><th className="p-2 md:p-3">Chuẩn</th><th className="p-2 md:p-3">Đã thu (trái)</th><th className="p-2 md:p-3">Quy đổi</th></tr>
              </thead>
              <tbody>
                {Object.keys(stats).map((customerName, index) => {
                  const data = stats[customerName];
                  const standardizedCount = data.total_harvest / data.standard_count;
                  return (
                    <tr key={index} className="border-b hover:bg-gray-50">
                      <td className="p-2 md:p-3 font-medium text-gray-800">{customerName}</td>
                      <td className="p-2 md:p-3 text-gray-600">{data.standard_count}</td>
                      <td className="p-2 md:p-3 font-bold text-green-600">{data.total_harvest.toLocaleString()}</td>
                      <td className="p-2 md:p-3 font-bold text-blue-600">{standardizedCount > 0 ? standardizedCount.toFixed(2) : 0} chuẩn</td>
                    </tr>
                  );
                })}
              </tbody>
